...
void Tutorial21_RayTracing::CreateRayTracingPSO()
{
    ...
    RefCntAutoPtr<IShader> pCubePrimaryHit, pGroundHit, pGlassPrimaryHit, pSpherePrimaryHit, pSphereSolida, pSphereGlass, pCubeSolid, pCubePrimary;
    {
        ...
        ShaderCI.Desc.ShaderType = SHADER_TYPE_RAY_CLOSEST_HIT;
        ShaderCI.Desc.Name       = "Cube solid";
        ShaderCI.FilePath        = "CubeSolid.rchit";
        ShaderCI.EntryPoint      = "main";
        m_pDevice->CreateShader(ShaderCI, &pCubeSolid);
        VERIFY_EXPR(pCubeSolid != nullptr);

        ShaderCI.Desc.ShaderType = SHADER_TYPE_RAY_CLOSEST_HIT;
        ShaderCI.Desc.Name       = "Cube primary";
        ShaderCI.FilePath        = "CubePrimary.rchit";
        ShaderCI.EntryPoint      = "main";
        m_pDevice->CreateShader(ShaderCI, &pCubePrimary);
        VERIFY_EXPR(pCubePrimary != nullptr);
    }
    ...
    PSOCreateInfo.AddTriangleHitShader("CubeSolid", pCubeSolid);

    PSOCreateInfo.AddTriangleHitShader("CubePrimary", pCubePrimary);
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    if (smallCubePositions.empty())
    {
        ...
        const int   gridSize = static_cast<int>(std::sqrt(MaxSmallCubes));
    ...
    for (int k = 0; k < 3; ++k)
    {
        ...
        inst.Transform.SetRotation(float3x3::Scale(2.0f, 2.0f, 2.0f).Data());
    ...
    static std::string smallCubesNames[MaxSmallCubes];
    constexpr int      IndexCube = BaseCount + MaxSmallSpheres;
    ...
    for (int i = 0; i < MaxSmallCubes; ++i)
    {
        ...
        smallCubesNames[i] = "SmallCubes" + std::to_string(i);
        inst.InstanceName  = smallCubesNames[i].c_str();
        inst.pBLAS         = m_pCubeBLAS;
        inst.CustomId      = i;
        ...
        inst.Transform.SetTranslation(pos.x + 2.0f, -5.32f, pos.z + 2.0f);

        inst.Transform.SetRotation(float3x3::Scale(0.6f, 0.6f, 0.6f).Data());
    }
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance", PRIMARY_RAY_INDEX, "GlassPrimaryHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance 2", PRIMARY_RAY_INDEX, "CubeSolid");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance 3", PRIMARY_RAY_INDEX, "CubePrimary");
    // clang-format on
    ...
    for (int i = 0; i < NumSmallSpheres; ++i)
    {
        ...
        const char* shaderName =
            (distribution(generator) < 7) ? "SphereSolida" :
            (distribution(generator) < 8) ? "SphereGlass" :
                                            "SpherePrimaryHit";
    ...
    {
        std::string instanceName = "SmallCubes" + std::to_string(i);

        const char* shaderName =
            (distribution(generator) < 7) ? "CubeSolid" :
            (distribution(generator) < 8) ? "GlassPrimaryHit" :
                                            "CubePrimary";
...
void Tutorial21_RayTracing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    // Initialize constants.
    {
        ...
        // Sphere constants.
        m_Constants.SphereReflectionColorMask = {0.6706f, 0.6706f, 0.6706f};
        m_Constants.SphereReflectionBlur      = 1;

        // Glass cube constants.
        m_Constants.GlassReflectionColorMask = {0.9f, 0.9f, 0.9f};
        ...
        m_Constants.LightColor[0] = {1.0f, 1.0f, 1.0f, 0.f}; 
        m_Constants.LightColor[1] = {1.0f, 1.0f, 1.0f, 0.f};
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        {
            m_ScatterCubesPositions.clear();
            m_ScatterCubesNames.clear();
            m_ScatterCubesPositions.reserve(m_NumScatterCubes);
            m_ScatterCubesNames.reserve(m_NumScatterCubes);
            ...
            {
                m_ScatterCubesPositions.emplace_back(distX(m_Rng), yFloor, distZ(m_Rng));
                m_ScatterCubesNames.push_back("Cube" + std::to_string(i));
            }
        }
